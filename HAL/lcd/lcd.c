               ...
               gpioPins_et D7 )
{
  /* Copy the PIN numbers where the LCD is connected */
    ...
    {
        LCDConfig.v_LcdMode_U8 = C_FourBitMode_U8; // Select 4-bit mode as D0-D3 are not used(P_NC)
    }
    else
    {
        LCDConfig.v_LcdMode_U8 = C_EightBitMode_U8; // 8-bit mode configure D0-D3 as output.
